...
# loading/decoding hints). Add them post-render: decode async on every content
# image, and lazy-load all but the first so the likely LCP image isn't deferred.
class Builders::Inspectors::Images < SiteBuilder
  def build
    return if Bridgetown.env.development?

    inspect_html do |document|
      document.query_selector_all("main img").each_with_index do |img, index|
