...
`fnox` decrypts `fnox.toml` (committed) with an age key (not committed) at `~/.config/fnox/age.txt`. Without that key, `mise run build` cannot read configured secrets.
...
2. On the new machine, verify decryption:
   ...
   ```
3. If the source machine is gone and the key is lost, generate a new age identity and re-encrypt `fnox.toml` against it; you'll need to re-add every secret from its source-of-truth (Cloudflare dashboard, Buzzsprout, etc.).
