...
Personal site built with [Bridgetown](https://www.bridgetownrb.com), styled with Tailwind v4 and [Radix UI Colors](https://www.radix-ui.com/colors) (Sage + Mint). Syntax highlighting via Bridgetown's built-in Rouge.
...
After this, `mise run build` wraps the production build in `fnox exec` so configured secrets are in scope for build/deploy tasks.
