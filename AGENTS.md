...
- `mise run dev` — start the Bridgetown dev server (`bin/bridgetown start`). Dev intentionally **skips** Torchlight and OG image generation (code blocks render plain; OG `<meta>` tags emit predicted `/og/<slug>.png` paths but the PNG files only exist after a production build, so OG previews look broken in dev until `mise run build` is run at least once).
- `mise run build` — production build wrapped in `fnox exec` so `TORCHLIGHT_TOKEN` from `fnox.toml` is exported. Equivalent to `BRIDGETOWN_ENV=production bin/bridgetown deploy`.
...
- **`og_images.rb`** — `resources:post_read` hook assigns `resource.data.image = "/og/<slug>.png"` for posts/projects/pages/cfps (skipping prototype pages and any with `image:` already set). In production only, a `site:post_write` hook (priority `:low`, runs after Torchlight) shells out to `scripts/generate-og.mjs`, which parses every `output/**/*.html`, extracts `og:title`/`og:description`/`og:image`, and writes PNGs to `output/og/<slug>.png` via Satori + Resvg. Generator reads **rendered HTML** (per PRD), so any computed title or `seo_title` override flows through.
- **`torchlight.rb`** — `site:post_write` hook in production only. Runs `pnpm exec torchlight highlight -c torchlight.config.cjs -i output`. The `-i output` flag and explicit `.cjs` config path are load-bearing (CLI auto-discovery misbehaves with `"type": "module"`).
...
- `src/_posts/_defaults.yml` sets `layout: post_layout`, `torchlight: true`, and a `back:` breadcrumb. Subdirectories like `src/_posts/blog/` and `src/_posts/snippets/` have their own `_defaults.yml` that can override.
...
- `TORCHLIGHT_TOKEN` is required for production builds (highlighting). In local dev it's loaded by `fnox` via `mise run build`; in CI it's a GitHub repo secret.
- `BRIDGETOWN_ENV=production` toggles Torchlight, OG image generation, and the link inspector. Tests use `BRIDGETOWN_ENV=test`.
...
`.github/workflows/ci.yml` runs four jobs on push/PR: `standardrb` lint, `bundle exec rake test`, a full production build (exercises OG generation + Torchlight via repo secret `TORCHLIGHT_TOKEN`), and a deploy job gated on the other three. The site is deployed to **Cloudflare Workers Static Assets** (see `wrangler.jsonc`) via `cloudflare/wrangler-action@v4`:
...
Required GitHub repo secrets: `TORCHLIGHT_TOKEN`, `CLOUDFLARE_API_TOKEN` (Workers Scripts: Edit permission), `CLOUDFLARE_ACCOUNT_ID`.
