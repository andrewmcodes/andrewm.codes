...
- **Stale Amber** (Radix `amber-2` fill, `amber-6` border, `amber-11` icon) — _borrowed semantic_: the stale-post notice only, raised on posts whose last update is more than two years old. Allowed because "this may be out of date" is a caution the brand red would misreport. The fill is step **2**, not 3: at step 3 the site's one warning became the heaviest object in the reading column, which inverts the point of a quiet aside. Like every other colour in the system it is a light/dark pair, so the notice flips with the theme.
...
- **Read** (Source Serif 4, `18px`, `1.7`): the reading column and nothing else. Constrained to a **65ch measure** (619px at this size), chosen in-browser against real character width. `Prose` emits exactly one max-width class — `max_w: false` means "let the reading measure govern" and yields `max-w-prose`; `true` yields `max-w-none` — so the measure is one decision rather than two competing classes resolved by compile order.
...
A single centred column with a label rail, one gutter, no sidebars. `PageShell` owns page width in three sizes: **wide** (`1080px`, `36px` gutter) for the homepage and listings, **reading** (`980px`) for posts — wide enough to hold its own apparatus in the margin while the column beside it stays at a reading measure, not wide enough for the prose itself to grow — and **narrow** (`720px`, `32px` gutter) for simple pages. All collapse to a `16px` gutter below `768px`. The topbar and footer independently repeat the wide shell's `1080px / 36px` measurements, so all three align to the same edge at every width. The reading shell is `980px` rather than `940px` because at `980px` the article column resolves to **619px, a true 65ch**; at `940px` the grid column rather than the `max-w-prose` token was the binding constraint and the measure had quietly fallen to 63ch — a page width is a type decision here, not just a container.
...
`slate-6` was chosen for the hairline by measurement: 1.37:1 in light and 1.65:1 in dark against the ground, where `slate-4` measured 1.19 / 1.31 and disappeared. One hairline per boundary, never two: because every `Entry` carries its own bottom rule, an archive group's last row plus the next group's top rule doubled every boundary with dead space between them, so `CollectionGroup` strips the final child's border (`[&>*:last-child]:border-b-0`) and the group's own top rule is the boundary. A one-row group made it obvious; it was happening at all of them. Tiles are not boxes; they are regions bounded by rules, which is why `.card-grid` draws its hairlines on the children (`border-bottom`, `border-right` on odd cells) rather than using a coloured gap — an odd card count must not leave a filled cell behind. `Entry` gets its hover surface from a `::before` inset `-12px` horizontally and `4px` vertically at `-z-10` with a 6px radius, so a row tints without becoming a box and the separator stays the hairline.

### Shadow Vocabulary

Exactly **two** shadows survive, and the whole vocabulary is these two:

- **Popover** (`shadow-lg`: `0 10px 15px -3px rgb(0 0 0 / .1), 0 4px 6px -4px rgb(0 0 0 / .1)`): the Markdown-actions dropdown on a post. It earns the shadow by being the one thing on the site that genuinely floats _free_ — it is summoned, it overlaps content it does not belong to, it is dismissed, and it has no scrim to separate it from the reading column underneath. The palette and the drawer each get their separation from a full-bleed scrim and a border, so neither needs one. This is also the only shadow inside the reading column.
...
A figure image is the instructive counter-example: it used to carry `shadow-lg`, and it lost it, because a figure scrolls with the document. It is bounded by a `ring-1 ring-slate-6` hairline instead — the same answer the rest of the system gives.
...
`PostAside` is the reading page's margin: a mono `<dl>` of date, reading time, and byline (a vertical stack in the rail, re-flowing to a horizontal wrapped row below `nav`), then tags, then contents. Sticky at `top-24` on desktop, spanning both grid rows. Below `nav` it sits between the title and the article rather than above both — the apparatus must not be the first thing a reader meets after tapping a headline. Its grid placement is passed in as a class by the layout, so the component owns its content and the page owns where it goes.

Below `nav` the contents is a disclosure, and its `<summary>` carries a chevron that rotates 90° on `group-open`. The chevron is not decoration: the summary's type recipe — mono, 12px, uppercase, `0.08em`, slate-11 — is identical to five _non-interactive_ labels on the site (`now`, `featured work`, `index`, `tags`, and the desktop contents heading), so without an affordance the post's only navigation control on mobile gave no sign it was a control. It rotates from the open state in CSS rather than from JS. Deep links land correctly because `.prose` headings carry `scroll-margin-top: 5.5rem`, clearing the sticky header they would otherwise land under.
...
- **Shape:** `6px`, or a full circle when `circle: true`. Five sizes from `xs` to `xl`; every size above `sm` shares the same `13.5px` label, so the scale is padding, not type. Every non-share variant carries `group`, so a child icon's `group-hover:` nudge actually fires — without it the hero's arrow was styled to move and never did.
