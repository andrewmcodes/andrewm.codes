...
//
// Shipped with Bridgetown v2.2.0
  ...
  setup: (build) => {
    ...
    build.onLoad({ filter: /.*/, namespace: "import-glob" }, async (args) => {
      const files = fsLib
        ...
        .sort()
        .map((module) =>
          module.replace(`../../${bridgetownConfig.source}/${bridgetownConfig.componentsDir}/`, "$components/"),
        )
...
export default async (esbuildOptions, ...args) => {
  ...
  const islands = fsLib
    .globSync(`./${bridgetownConfig.source}/${bridgetownConfig.islandsDir}/*.{js,js.rb}`)
    .map((item) => `./${item}`);
